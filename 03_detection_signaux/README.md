...
son site (accueil, actualités, carrières) et demande à un moteur IA
(Claude ou Ollama en local, gratuit) d'en extraire un signal d'achat B2B
(recrutement IA/data, levée de fonds, nouveau produit...). Si le site est
inaccessible ou qu'aucun signal n'est trouvé, la ligne est marquée
explicitement (pas d'invention).
...
Nécessite `ANTHROPIC_API_KEY` dans `.env` (moteur par défaut). Avec
Ollama en local (gratuit) : ajouter `--moteur ollama`.
